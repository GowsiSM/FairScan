...
def prepare_binary_dataset(
	...
	*,
	...
	if label_col not in df.columns:
		raise HTTPException(status_code=400, detail=f"label_col '{label_col}' not found in CSV.")
	if sensitive_col not in df.columns:
		raise HTTPException(status_code=400, detail=f"sensitive_col '{sensitive_col}' not found in CSV.")
	...
	if positive_label not in unique_labels:
		raise HTTPException(status_code=400, detail=f"positive_label '{positive_label}' not found in {label_col}.")
	if privileged_val not in unique_sensitive:
		raise HTTPException(status_code=400, detail=f"privileged_val '{privileged_val}' not found in {sensitive_col}.")
	...
	if inferred_unpriv is None:
		...
		if not candidates:
			raise HTTPException(status_code=400, detail="Need at least two groups in sensitive column.")
	...
	if inferred_unpriv == privileged_val:
		raise HTTPException(status_code=400, detail="privileged_val and unprivileged_val must differ.")
	...
	if df_filtered.empty:
		raise HTTPException(status_code=400, detail="No rows remain after group filtering.")
...
def _equal_opp_diff_from_model(prepared: PreparedData, sample_weight: np.ndarray | None = None) -> float:
	df = prepared.df_original.copy()
	label_binary = (_as_text(df[prepared.label_col]) == prepared.positive_label).astype(int).to_numpy()
...
def compute_metrics(prepared: PreparedData) -> dict[str, Any]:
	...
	label_text = _as_text(prepared.df_original[prepared.label_col])

	priv_mask = group_text == prepared.privileged_val
	unpriv_mask = group_text == prepared.unprivileged_val

	priv_rate = float((label_text[priv_mask] == prepared.positive_label).mean()) if priv_mask.any() else 0.0
	unpriv_rate = (
		float((label_text[unpriv_mask] == prepared.positive_label).mean()) if unpriv_mask.any() else 0.0
	)
...
def apply_reweighing(prepared: PreparedData) -> tuple[pd.DataFrame, dict[str, Any]]:
	...
	eod_after = _equal_opp_diff_from_model(prepared, sample_weight=dataset_fixed.instance_weights)
